根据您提供的图片文件名（按时间顺序排序），我已将所有文本内容识别并整合为一份完整的 Markdown 文档。以下是恢复后的内容：
...
- **LlmRequest** / `LlmResponse` / `google.genai.ttypes`: 用于描述模型请求、模型响应和消息内容。
...
- **instruction**: 系统任务说明，即 prompt。可以写普通文本，也可以使用 state 变量占位。
- **tools**: 工具函数列表。
...
def search_knowledge_base(tool_context: ToolContext) -> str:
    ...
    records = [
        {"title": "Password reset", "content": "Use the reset link on the sign-in page."},
        {"title": "MFA setup", "content": "Open settings and add an authenticator app."},
    ]
...
If the answer needs knowledge_base information, call search_knowledge_base first.
""".strip(),
...
`SequentialAgent` 会按 `sub_agents` 的顺序逐个运行子 agent。前一个 agent 写入 state 的结果，可以被后一个 agent 使用。
...
`ParallelAgent` 会并行运行多个 agent。它适合把互相独立的任务同时执行，提高吞吐和整体速度。
        ...
        for index, batch_text in enumerate(batches):
            ...
            sub_agents.append(
                Agent(
                    ...
                    instructions=self.SUMMARY_TEMPLATE.replace("{batch_text}", f"{{{state_key}}}"),
...
在自定义 `BaseAgent` 里，它是读取输入、创建动态子 agent、传递上下文的关键对象。
...
class NoopAgent(BaseAgent):
    name: str = "noop_agent"
    description: str = "Record that this stop was reached."

    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ctx.session.state["noop_reached"] = True
        return
        yield  # This is needed to make it an async generator, though it yields nothing.
```
...
- **Runner** 负责把 root_agent、session_service 和 app 名称连接起来，并启动运行。
...
不同 ADK patch 版本对 `LlmResponse.content` 的内部结构可能有差异。如果你遇到类型错误，可以改用 `types.GenerateContentResponse` 或 `types.Candidate` 的形式。
...
| **Agent** | 定义普通 LLM 智能体 | 单任务、工具调用 |
| **LlmAgent** | 显式定义 LLM-backed agent | 工具型 agent，明确表达 LLM agent |
| **SequentialAgent** | 顺序运行多个 agent | 多步骤 pipeline |
| **ParallelAgent** | 并行运行多个 agent | 多批次、多角度、多分支处理 |
...
| **CallbackContext** | 回调函数访问 state 和上下文 | before/after callback 参数 |
